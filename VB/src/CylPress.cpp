...
void RK4c(double *P, double *dP, int n, double *phiR, double htry, double *Pscal, double *hdid, double *hnext, void (*Derivs)(double, double *, double *, float *), float *Baleakage)
{
	int i, jtry;	
...
/* dP/dphi */
void Derivs(double phid, double *Pt, double *dP, float *Baleakage)
{
	...
	for(i=1; i<=N; i++){
		...
		if(Nd >= 1){
			Qcb[i] = Baleakage[i];
			//printf("%e %e \n", Baleakage[i], pow(hv,3)/12/eta*(Pt[i]-Ph)*(1/log(1+LB1/(R-rB))+1/log(1+LB2/(R+rB)))*2*(thetaB));
		}
		else Qcb[i] = pow(hv,3)/12/eta*(Pt[i]-Ph)*(1/log(1+LB1/(R-rB))+1/log(1+LB2/(R+rB)))*2*(thetaB);		
		...
		dP[i] = -K/w/(V0-R*(tan(beta)*(1-cos(phiD[i]))+tan(beta2)/cos(beta)*sin(phiD[i]))*Ap)
				*( -w*R*(tan(beta)*sin(phiD[i])+tan(beta2)/cos(beta)*cos(phiD[i]))*Ap 
				+ Cvo*Avo(phiD[i])*sqrt(2/rho)*sqrt(fabs(Pt[i]-Pt[N+1]))*sgn1 + Cvi*Avi(phiD[i])*sqrt(2/rho)*sqrt(fabs(Pt[i]-Pt[N+2]))*sgn3
