...
void Input()
{
	...
	if((ep = fopen("Input_para.dat", "r"))==NULL) {
		printf("Cannot open file.\n");
		exit(1);
	}

	fscanf(ep, "%*s %f %*s %f %*s %f %*s %f %*s %f %*s %f %*s %f %*s %f %*c %f %*c %f %*c %f %*c %f %*c %f %*s %f %*s %f %*c %f %*s %f %*c %f	\
				%*s %f %*c %f %*s %f %*s %f %*s %f %*s %f %*s %f %*s %f %*s %f %*s %f %*c %f %*s %f %*s %f %*c %f %*s %f %*c %f %*s %f %*c %f %*s %f %*s %f %*s %f	\
				%*s %f %*s %f %*c %f %*c %f %*c %f %*c %f %*s %f %*s %d %*c %d %*c %d %*c %d %*c %d %*s %d %*s %f %*c %f %*c %f %*c %f %*c %f %*c %f %*s %d %*s %f %*s %f %*s %f %*c %f %*c %f %*c %f %*c %f %*c %f %*s %f %*s %f %*s %f %*c %f %*c %f %*s %f %*c %f %*c %f %*c %f %*c %f", 
...
void initial(float **Pvb, double *ym, double **Hvb, double **Hvb_gro, float **Rvb, float **Tvb, double *Pc, float *Baleakage)
{
	...
	Pc[10] = Pout*1.0;
	Pc[11] = Pin*1.0;
	Pc[12] = 0.000000015/*0.1*w*R*(tan(beta)*sin(2*Pi/9)+tan(beta2)/cos(beta)*cos(2*Pi/9*2))*Ap*/;
	Pc[13] = 0.0000000015;
...
void groove(double **H_gro, float **Tvb)
{
	...
	float Wh1 = 1.0e-6/hv0, Wh2 = 1.0e-6/hv0, Wh3 = 1.0e-6/hv0, Wh5 = 0.0e-6/hv0, Rv;
	...
	Rc1 = rB1;
	a1 = Wh1/((rB2*rB2-rB1*rB1)-2*Rc1*(rB2-rB1));
	b1 = -2*Rc1*a1;
	c1 = -a1*rB1*rB1-b1*rB1;
	Rc3 = rB4;
	a3 = Wh3/((rB3*rB3-rB4*rB4)-2*Rc3*(rB3-rB4));
	b3 = -2*Rc3*a3;
	c3 = -a3*rB4*rB4-b3*rB4;
	Rc5 = rB5;
	a5 = Wh5/((rB6*rB6-rB5*rB5)-2*Rc5*(rB6-rB5));
	b5 = -2*Rc5*a5;
	c5 = -a5*rB5*rB5-b5*rB5;

	for(i=1; i<1+NBr1; i++){
		for(j=1; j<=NNBt; j++){
			H_g[i][j] = a1*pow(rB1+(i-1)*((rB2-rB1)/NBr1),2)+b1*(rB1+(i-1)*((rB2-rB1)/NBr1))+c1;
			H_gro[i][j] = H_gro[i][j] + H_g[i][j]; 		
		}
	}

	for(i=1+NBr1; i<1+NBr1+NBr2; i++){
		for(j=1; j<=NNBt; j++){
			H_g[i][j] = Wh2;
			H_gro[i][j] = H_gro[i][j] + H_g[i][j]; 
		}
	...
	for(i=1+NBr1+NBr2; i<=1+NBr1+NBr2+NBr3; i++){
		for(j=1; j<=NNBt; j++){
			H_g[i][j] = a3*pow(rB3+(i-1-NBr1-NBr2)*((rB4-rB3)/NBr3),2)+b3*(rB3+(i-1-NBr1-NBr2)*((rB4-rB3)/NBr3))+c3;
			H_gro[i][j] = H_gro[i][j] + H_g[i][j]; 
		}
	}
	...
	for(i=1+NBr1+NBr2+NBr3+NBr4; i<=1+NBr1+NBr2+NBr3+NBr4+NBr5; i++){
		for(j=1; j<=NNBt; j++){
			H_g[i][j] = a5*pow(rB5+(i-1-NBr1-NBr2-NBr3-NBr4)*((rB6-rB5)/NBr5),2)+b5*(rB5+(i-1-NBr1-NBr2-NBr3-NBr4)*((rB6-rB5)/NBr5))+c5;
			H_gro[i][j] = H_gro[i][j] + H_g[i][j]; 
		}
	}
