...
void FrictionPC(double phi, float *Ffp, float *Mfxp, float *Mfyp, float **Zpc, float **Tpc, double **Hpc, float **Acv, float **dPdZ_n, float **Ppc)
{
	...
	for(j=1; j<=NNBt; j++){
		for(i=1; i<=NNBz; i++){			
			...
			Ffp_cv[i][j] = (hp/Rp)/6*(Vp/Hpc[i][j]+3.0*Hpc[i][j]*dPdZ_n[i][j])*Acvf[i][j];
			Mfxp_cv[i][j] = -(hp/Rp)/6*(Vp/Hpc[i][j]+3.0*Hpc[i][j]*dPdZ_n[i][j])*Acvf[i][j]*cos(Tpc[i][j]);
			Mfyp_cv[i][j] = (hp/Rp)/6*(Vp/Hpc[i][j]+3.0*Hpc[i][j]*dPdZ_n[i][j])*Acvf[i][j]*sin(Tpc[i][j]);
	...
	*Mfyp = 0.0;  
	...
	frictionD = (*Ffp)*(eta*w*pow(Rbo,4)/hp);
