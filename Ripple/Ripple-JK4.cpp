...
double g=9.8, Rol=12.7e-3, Rpipe=12.7e-3, Aol, Lol=200e-3, Lpipe=1000e-3, Apipe;
int PCYCLE=0, Nol=2000, Np=10000;
	...
	while(phi<phif){		
		...
		if(phi>2*Pi){		
			if(hout>=phiout){
				...
				fprintf(fp,"%e %e %e %e ", Pol[1], Pp[1], Pol[Nol], Pp[Np]);
				//printf("%e %e ", Pol[1], Pp[1]);
				fprintf(fp,"%e ", Qvo);
		...
		if(phi>2*Pi){
			...
			//double phit = (olphi>=pphi?pphi:olphi); 
...
void Input()
{
	...
	if((ep = fopen("Input_pressure_JKKIM.dat", "r"))==NULL) {
		printf("Cannot open file.\n");
		exit(1);
	}
...
void Init(double *P, double *Pp, double *Qp, double *Pol, double *Qol)
{
	...
	for(i=1; i<=Np; i++){
		...
		Qp[i] = 0.0/*Ap*2*R*tan(beta0)*N*w/60*(1+0.01*sin((i-1)*2*Pi))*/;
	}
	for(i=1; i<=Nol; i++){
		Pol[i] = Pout/*+0.001*Pout*sin((i-1)*2*Pi)*/;
		Qol[i] = 0.0/*Ap*2*R*tan(beta0)*N*w/60*(1+0.001*sin((i-1)*2*Pi))*/;
	}
...
void Derivs(double phid, double *Pt, double *dP, double *Pol, double *Qol)
{
	...
	double Cv, *Pr, Pv, *phiD, *Lpc, Ao1, Ao2, Qvo, Qvi,Apo, Lvp=50e-3, Ain, Aout, bv1, bv2, bvo=100e-3, Lv1=500e-3, Rpipei=6e-3, Lv2=500e-3, Lv3=100e-3, Lv4=1000e-3, tanv, Bv, Cvpo;
	...
	//sgn1 = (Pt[N+1]>=Pin?1:-1);
	dP[N+1] = Kt(Pt[N+1])/w/((bv1-Lvp*tanv)*Lvp*2*rV1)*(Qvo - Qol[1]/*Pt[N+3]*//*Cvo*11.5e-6*sqrt(2/rho)*sqrt(fabs(Pt[N+1]-Pin))*sgn1*/);
	//dP[N+3] = Ain/Lv3/w/rho*((Pt[N+1]-Pol[1]) - 3/pow(Ain,2)*rho/2*Pt[N+3]*fabs(Pt[N+3]) /*- 3/pow(Aout*0.02,2)*rho/2*Pt[N+3]*fabs(Pt[N+3])*/ );
...
void Outlet(double phi, double *Pol, double *Qol, double *P, double *Pp, double *Qp)
{
	...
	//Qol[Nol] = Qp[1];
	//Pol[Nol] = rho*g*(Pol[Nol-1]/rho/g+BL*Qol[Nol-1]-Res*Qol[Nol-1]*fabs(Qol[Nol-1])-BL*Qp[1]);
	//Pol[Nol] = Pp[1];
	//Qol[Nol] = -Pol[Nol]/rho/g/BL+(Pol[Nol-1]/rho/g+BL*Qol[Nol-1]-Res*Qol[Nol-1]*fabs(Qol[Nol-1]))/BL;
	//Pol[Nol] = Pout;
	//Qol[Nol] = -Pol[Nol]/rho/g/BL+(Pol[Nol-1]/rho/g+BL*Qol[Nol-1]-Res*Qol[Nol-1]*fabs(Qol[Nol-1]))/BL;
	alpha = (Pp[1]-Cp*rho*g>=0? 0.6*11.8e-12*sqrt(2/rho):0.6*11.8e-2*sqrt(2/rho));
	int sgn = (Pp[1]-Cp*rho*g>=0?-1:1);
	Qol[Nol] = (-alpha*alpha*BL*rho*g+sgn*sqrt(pow(alpha*alpha*BL*rho*g,2)-sgn*4*alpha*alpha*(Pp[1]-Cp*rho*g)))/2;
	...
	//Pol[Nol] = rho*g*(Cp-(BL+BL2)/2*Qol[Nol]);
...
void Pipe(double phi, double *Pp, double *Qp, double *P, double *Pol, double *Qol)
{
	...
	Pp[Np] = Pout;
	Qp[Np] = -Pp[Np]/rho/g/BL+(Pp[Np-1]/rho/g+BL*Qp[Np-1]-Res*Qp[Np-1]*fabs(Qp[Np-1]))/BL;
}
