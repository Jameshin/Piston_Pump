...
double g=9.8, Rol=12.7e-3, Rpipe=4.7e-3, Aol, Lol=1000e-3, Lpipe=5e-3, Apipe;
int PCYCLE=0, Nol=2000, Np=10;
double c, c2, Ep=200e9, ep=20.5e-3, ep2=6.5e-3, pphi, olphi, phiout_p=30;
double An, thetaN, Cv, beta;
int nn1=0, nn2=0;
...
int main()
{
	...
	while(phi<phif){		
		...
		if(phi>0*Pi){		
			if(hout>=phiout){
				fprintf(fp,"%4.1f  ", (phi-0*Pi)*180/Pi);
				for(i=1; i<=NRK; i++) fprintf(fp,"%e  ", P[i]);
				//Qvo = P[N+3];
...
/*			if(hout_p>=phiout_p){
				...
				for(i=1; i<=Nol; i++) fprintf(gp,"%d  %e %e \n", i, Pol[i], Qol[i]);
				PCYCLE++;
				hout_p = 0.0;
			}*/			
		}
...
void Input()
{
	...
	if((ep = fopen("Input_pressure_JKKIM2.dat", "r"))==NULL) {
		printf("Cannot open file.\n");
		exit(1);
	}
...
void Init(double *P, double *Pp, double *Qp, double *Pol, double *Qol)
{
	...
	P[10] = Pout*1.0;
	P[11] = Pin*1.0;
	P[12] = 0.0/*Ap*2*R*tan(beta0)*N*w/60*/;
	P[13] = 0.0/*Ap*2*R*tan(beta0)*N*w/60*/;
	P[14] = 0.0/*0.0000000015*/;
	P[15] = 0.0;
	P[16] = Ap*2*R*tan(beta0)*N*w/60/*0.0000000015*/;
...
void Derivs(double phid, double *Pt, double *dP, double *Pol, double *Qol)
{
	...
	double *Pr, Pv, *phiD, *Lpc, Ao1, Ao2, Qvo, Qvi,Apo, Lvp=200e-3, Ain, Aout, bv1, bv2, bvo=20e-3, Lv1=500e-3, Rpipei=20e-3, Lv2=500e-3, Lv3=1000e-3, Lv4=500e-3, tanv, Bv, Cvpo;
	double dbeta, wd, wn=60*2*Pi, zeta=1/1.414/2;
	double Fin, Fp;
	...
	for(i=1; i<=N; i++){
		...
		//if((phiD[i]>thetaN-thetaB && phiD[i]<thetaV1-thetaB) || (phiD[i]>2*Pi-thetaV6+thetaB && phiD[i]<2*Pi-thetaN+thetaB)){
		//	if(nn1 != i){
		//		Pt[N+3] = 0.0;
		//		Pt[N+4] = 0.0;
		//	}
		//	if((phiD[i] > 0 && phiD[i] < thetaN-thetaB) || (phiD[i] > 2*Pi+thetaN-thetaB && phiD[i] < 2*Pi)){
		//		Pt[N+3] = 0.0;
		//		dP[N+3] = 0.0;
		//		dP[N+4] = 1/w*( (Pt[N+2]-Pt[i])/rho - Pt[N+4]*fabs(Pt[N+4])/(pow(An*(0.01+(thetaV6-thetaN-phiD[i]+2*Pi-thetaV6+thetaB)/(thetaV6-thetaN)),2))/2/Cv/Cv)/(R*(thetaV6-thetaN)/An*(log(1.01*(thetaV6-thetaN)/(thetaV6-thetaN-phiD[i]+2*Pi-thetaV6+thetaB))));
		//	}
		//	else if((phiD[i] >= 2*Pi-thetaN+thetaB && phiD[i] < 2*Pi) || (phiD[i] > 0 && phiD[i] < -thetaN+thetaB)){
		//		dP[N+3] = 1/w*( (Pt[N+1]-Pt[i])/rho - Pt[N+3]*fabs(Pt[N+3])/(pow(An*(0.01+(phiD[i]+thetaB-thetaN)/(thetaV1-thetaN)),2))/2/Cv/Cv)/(R*(thetaV1-thetaN)/An*(log(1.01*(thetaV1-thetaN)/(phiD[i]+thetaB-thetaN))));
		//		Pt[N+4] = 0.0;
		//		dP[N+4] = 0.0;
		//	}
		//	else{				
		...
		//	}
			//dP[i] = -Kt(Pt[i])/w/(V0-R*(tan(beta)*(1-cos(phiD[i]))+tan(beta2)/cos(beta)*sin(phiD[i]))*Ap)
			//		*( -R*(dbeta/pow(cos(beta),2)*(1-cos(phiD[i]))+dbeta*tan(beta2)*sin(beta)/cos(beta)/cos(beta)*sin(phiD[i])+w*(tan(beta)*sin(phiD[i])+tan(beta2)/cos(beta)*cos(phiD[i])))*Ap 
			//		- Pt[N+3] - Pt[N+4] + Cv*Avo(phiD[i])*sqrt(2/rho)*sqrt(fabs(Pt[i]-Pt[N+1]))*sgn1 + Cv*Avi(phiD[i])*sqrt(2/rho)*sqrt(fabs(Pt[i]-Pt[N+2]))*sgn3
			//		+ 2*Pi*Rp*pow(hp,3)/12/eta/Lpc[i]*(Pt[i]-Ph)-2*Pi*Rp/2*hp*R*(dbeta/pow(cos(beta),2)*(1-cos(phiD[i]))+w*(tan(beta)*sin(phiD[i])+tan(beta2)/cos(beta)*cos(phiD[i])))
			//		+ pow(hv,3)/12/eta*(Pt[i]-Ph)*(1/log(1+LB1/(R-rB))+1/log(1+LB2/(R+rB)))*2*thetaB 
			//		+ Cs*Pi*Rso*Rso*sqrt(2/rho)*sqrt(fabs(Pt[i]-Pr[i]))*sgn2 );
			//nn1 = i;
			//printf("%e \n ",Pt[N+3]);	
		//}
		//else if((phiD[i]>Pi-thetaV3+thetaB && phiD[i]<Pi-thetaN+thetaB) || (phiD[i]>Pi+thetaN-thetaB && phiD[i]<Pi+thetaV4-thetaB)){
		//	if(nn2 != i){
		//		Pt[N+5] = 0.0;
		//		Pt[N+6] = 0.0;
		//	}
		//	if(phiD[i] > Pi-thetaN+thetaB){
		//		Pt[N+5] = 0.0;
		//		dP[N+5] = 0.0;
		//		dP[N+6] = 1/w*( (Pt[N+2]-Pt[i])/rho - Pt[N+6]*fabs(Pt[N+6])/(pow(An*(0.01+(thetaV6-thetaN-phiD[i]+2*Pi-thetaV6+thetaB)/(thetaV6-thetaN)),2))/2/Cv/Cv)/(R*(thetaV6-thetaN)/An*(log(1.01*(thetaV6-thetaN)/(thetaV6-thetaN-phiD[i]+2*Pi-thetaV6+thetaB))));
		//	}
		//	else if(phiD[i] <= Pi+thetaN-thetaB){
		//		dP[N+5] = 1/w*( (Pt[N+1]-Pt[i])/rho - Pt[N+5]*fabs(Pt[N+5])/(pow(An*(0.01+(phiD[i]+thetaB-thetaN)/(thetaV1-thetaN)),2))/2/Cv/Cv)/(R*(thetaV1-thetaN)/An*(log(1.01*(thetaV1-thetaN)/(phiD[i]+thetaB-thetaN))));
		//		Pt[N+6] = 0.0;
		//		dP[N+6] = 0.0;
		//	}
		//	else{
		//		dP[N+5] = 1/w*( (Pt[N+1]-Pt[i])/rho - Pt[N+5]*fabs(Pt[N+5])/(pow(An*(0.01+(thetaV3-thetaN-phiD[i]+thetaB+thetaV1+thetaV2)/(thetaV3-thetaN)),2))/2/Cv/Cv)/(R*(thetaV3-thetaN)/An*(log(1.01*(thetaV3-thetaN)/(thetaV3-thetaN-phiD[i]+thetaB+thetaV1+thetaV2))));
		//		dP[N+6] = 1/w*( (Pt[N+2]-Pt[i])/rho - Pt[N+6]*fabs(Pt[N+6])/(pow(An*(0.01+(phiD[i]-Pi-thetaN+thetaB)/(thetaV4-thetaN)),2))/2/Cv/Cv)/(R*(thetaV4-thetaN)/An*(log(1.01*(thetaV4-thetaN)/(phiD[i]-Pi-thetaN+thetaB))));
		//	}
		//	dP[i] = -Kt(Pt[i])/w/(V0-R*(tan(beta)*(1-cos(phiD[i]))+tan(beta2)/cos(beta)*sin(phiD[i]))*Ap)
		//			*( -R*(dbeta/pow(cos(beta),2)*(1-cos(phiD[i]))+dbeta*tan(beta2)*sin(beta)/cos(beta)/cos(beta)*sin(phiD[i])+w*(tan(beta)*sin(phiD[i])+tan(beta2)/cos(beta)*cos(phiD[i])))*Ap 
		//			- Pt[N+5] - Pt[N+6] + Cv*Avo(phiD[i])*sqrt(2/rho)*sqrt(fabs(Pt[i]-Pt[N+1]))*sgn1 + Cv*Avi(phiD[i])*sqrt(2/rho)*sqrt(fabs(Pt[i]-Pt[N+2]))*sgn3
		//			+ 2*Pi*Rp*pow(hp,3)/12/eta/Lpc[i]*(Pt[i]-Ph)-2*Pi*Rp/2*hp*R*(dbeta/pow(cos(beta),2)*(1-cos(phiD[i]))+w*(tan(beta)*sin(phiD[i])+tan(beta2)/cos(beta)*cos(phiD[i])))
		//			+ pow(hv,3)/12/eta*(Pt[i]-Ph)*(1/log(1+LB1/(R-rB))+1/log(1+LB2/(R+rB)))*2*thetaB 
		//			+ Cs*Pi*Rso*Rso*sqrt(2/rho)*sqrt(fabs(Pt[i]-Pr[i]))*sgn2 );
		//	nn2 = i;
		//	//printf("%e \n ",Pt[N+3]);	
		//}
		//else if(phiD[i] == thetaV1-thetaB){
		//	dP[i] = -Kt(Pt[i])/w/(V0-R*(tan(beta)*(1-cos(phiD[i]))+tan(beta2)/cos(beta)*sin(phiD[i]))*Ap)
		//			*( -R*(dbeta/pow(cos(beta),2)*(1-cos(phiD[i]))+dbeta*tan(beta2)*sin(beta)/cos(beta)/cos(beta)*sin(phiD[i])+w*(tan(beta)*sin(phiD[i])+tan(beta2)/cos(beta)*cos(phiD[i])))*Ap 
		//			+ Cv*An*sqrt(2/rho)*sqrt(fabs(Pt[i]-Pt[N+1]))*sgn1 + Cv*Avi(phiD[i])*sqrt(2/rho)*sqrt(fabs(Pt[i]-Pt[N+2]))*sgn3
		//			+ 2*Pi*Rp*pow(hp,3)/12/eta/Lpc[i]*(Pt[i]-Ph)-2*Pi*Rp/2*hp*R*(dbeta/pow(cos(beta),2)*(1-cos(phiD[i]))+w*(tan(beta)*sin(phiD[i])+tan(beta2)/cos(beta)*cos(phiD[i])))
		//			+ pow(hv,3)/12/eta*(Pt[i]-Ph)*(1/log(1+LB1/(R-rB))+1/log(1+LB2/(R+rB)))*2*thetaB 
		//			+ Cs*Pi*Rso*Rso*sqrt(2/rho)*sqrt(fabs(Pt[i]-Pr[i]))*sgn2 );
		//}
	...
	//dP[N+1] = 0.0;
	//dP[N+2] = 0.0;
	//printf("%e \n", (bv1-Lvp*tanv)*Lvp*2*rV1);

	/* Simplest Shape with Piping */
	sgn1 = (Pt[N+1]>=Ph?1:-1);
	dP[N+1] = Kt(Pt[N+1])/w/((bv1-Lvp*tanv)*Lvp*2*rV1)*(Qvo - Qol[1]/*Pt[N+7]*//*Cvo*30.8e-6*sqrt(2/rho)*sqrt(fabs(Pt[N+1]-Ph))*sgn1*/);
	//dP[N+7] = Aout/Lv3/w/rho*((Pt[N+1]-Ph) - 1/Cvo/Cvo/pow(30.8e-6*1.2/*30.8e-6*/,2)*rho/2*Pt[N+7]*fabs(Pt[N+7]));
	...
	dP[N+2] = 0.0;
	//if(Pt[N+2]>Pout/2) dP[N+2] = Kt(Pt[N+2])/w/Vvi*(Qvi-Cvin*10.5e-6*sqrt(2/rho)*sqrt(Pt[N+2]-Pout/2));  
	//else dP[N+2] = Kt(Pt[N+2])/w/Vvi*(Qvi+Cvin*10.5e-6*sqrt(2/rho)*sqrt(Pout/2-Pt[N+2]));
...
{
	int i;
	double alpha, Cp, Cm, BL, zeta=0.2, Res, BL2, zeta2=0.2, Res2;
	//Rol = Rpipe;
	Aol = Pi*Rol*Rol;
...
void Pipe(double phi, double *Pp, double *Qp, double *P, double *Pol, double *Qol)
{
	...
	printf("---- \n");	
	alpha = 0.6*30.8e-6*sqrt(2/rho); /*15.8, 11.4,11.8, 8.8, 1.65*/
	BL = c2/g/Apipe;
	...
	//Pp[1] = Pol[Nol];
	//Qp[1] = Pp[1]/rho/g/BL-(Pp[2]/rho/g-BL*Qp[2]+Res*Qp[2]*fabs(Qp[2]))/BL;
	Qp[Np] = (-alpha*alpha*BL*rho*g+sqrt(pow(alpha*alpha*BL*rho*g,2)-4*alpha*alpha*(/*Pout*0.9*/Ph-(Pp[Np-1]/rho/g+BL*Qp[Np-1]-Res*Qp[Np-1]*fabs(Qp[Np-1]))*rho*g)))/2;
