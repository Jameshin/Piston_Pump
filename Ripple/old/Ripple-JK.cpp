...
double g=9.8, Rpipe=9.1e-3, Lpipe=10e-3, Apipe;
int Np=20, PCYCLE=0;
double c, Ep=200e9, ep=2e-3, pphi, phiout_p=30;
...
int main()
{
	...
	void Pipe(double , double *, double *, double * );
	...
	while(phi<phif){	
		...
		if(phi>0*Pi){		
			...
			if(hout_p>=phiout_p){
				...
				for(i=1; i<=Np; i++) fprintf(gp,"%d  %e %e \n", i, Pp[i], Qp[i]);
				PCYCLE++;
				hout_p = 0.0;
			}			
		}
		...
		hpipe += h*180/Pi;
		hout_p += h*180/Pi;
		Derivs(phi, P, dP, Pp, Qp);
		for(i=1; i<=NRK; i++) Pscal[i] = fabs(P[i]) + fabs(h*dP[i]) + TINY;
		RK4(Pp, Qp, P, dP, NRK, &phi, h, Pscal, &hdid, &hnext, Derivs);
...
void Input()
{
	...
	if((ep = fopen("Input_pressure_480.dat", "r"))==NULL) {
		printf("Cannot open file.\n");
		exit(1);
	}
...
/*Initial condition */
void Init(double *P, double *Pp, double *Qp)
{
	int i;
	...
	for(i=1; i<=Np; i++){
		Pp[i] = Pout+0.1*Pout*sin((i-1)*2*Pi);
		Qp[i] = Ap*2*R*tan(beta0)*N*w/60*(1+0.1*Pout*sin((i-1)*2*Pi));
	}
	
	w = w*2*Pi/60;	
	c = sqrt(K/rho)/sqrt(1+K*2*Rpipe/Ep/ep);
	pphi = w*Lpipe/c/Np;
}
...
/* R-K 4th */
void RK4(double *Pp, double *Qp, double *P, double *dP, int n, double *phiR, double htry, double *Pscal, double *hdid, double *hnext, void (*Derivs)(double, double *, double *, double *, double *))
{
	int i, jtry;	
...
/* dP/dphi */
void Derivs(double phid, double *Pt, double *dP, double *Pp, double *Qp)
{
	double Kt(double );

	int i, sgn1, sgn2, sgn3;
	double Cv, *Pr, Pv, *phiD, *Lpc, Ao1, Ao2, Qvo, Qvi,Apo, Lvp=100e-3, Ain, Aout, bv1, bv2, bvo=100e-3, Lv1=500e-3, Rpipei=20e-3, Lv2=500e-3, Lv3=500e-3, Lv4=500e-3, tanv, Bv, Cvpo;
	...
	Ain = 0.5e-6;
	sgn1 = (Pt[N+1]>=Pin?1:-1);
	sgn2 = (Pt[N+2]>=Pout?1:-1);
	dP[N+1] = Kt(Pt[N+1])/w/((bv1-Lvp*tanv)*Lvp*2*rV1)*(Qvo - /*Qp[1]*//*Pt[N+3]*/0.6*Ain*sqrt(2/rho)*sqrt(fabs(Pt[N+1]-Pin))*sgn1);
	//dP[N+3] = 1/rho/w/((bv1-Lvp*tanv)*Lvp*2*rV1)*(Fin-rho/Aout*Pt[N+3]*fabs(Pt[N+3])+Fp-Pp[1]*Aout+Pt[N+1]*(Aout-Ao1))*Aout - (Qvo-Pt[N+3])/w/((bv1-Lvp*tanv)*Lvp*2*rV1)*Pt[N+3];
	//dP[N+3] = 10*Aout/Lv3/w/rho*((Pt[N+1]-Pp[1]) - 3/pow(10*Aout,2)*rho/2*Pt[N+3]*fabs(Pt[N+3]) );
	//dP[N+3] = 4*Aout/Lv1/w/rho*( (Pt[N+1]-Pt[N+4]) - 10/pow(Aout,2)*rho/2*Pt[N+3]*fabs(Pt[N+3]) );
	//dP[N+4] = Kt(Pt[N+4])/w/Lv2/4/Aout*(Pt[N+3]-Pt[N+5]);
	//dP[N+5] = Aout/Lv3/w/rho*((Pt[N+4]-Pt[N+6]) - 3/pow(Aout,2)*rho/2*Pt[N+5]*fabs(Pt[N+5]) );
	//dP[N+6] = Kt(Pt[N+6])/w/Lv4/Aout*(Pt[N+5]-0.7*10e-6*sqrt(2/rho)*sqrt(Pt[N+6]-Ph));
	dP[N+2] = Kt(Pt[N+2])/w/Vvi*(Qvi-Cvin*Ain*sqrt(2/rho)*sqrt(fabs(Pt[N+2]-Pout))*sgn2);  
...
/* Pipe Pulse */
//void Pipe(double phi, double *Pp, double *Qp, double *P)
//{
//	int i;	
...
//	//Pp[1] = P[N+1];
//	//Qp[1] = P[N+1]/rho/g/BL-(Pp[2]/rho/g-BL*Qp[2]+Res*Qp[2]*fabs(Qp[2]))/BL;
//	Qp[Np] = (-alpha*alpha*BL*rho*g+sqrt(pow(alpha*alpha*BL*rho*g,2)-4*alpha*alpha*(Ph-(Pp[Np-1]/rho/g+BL*Qp[Np-1]-Res*Qp[Np-1]*fabs(Qp[Np-1]))*rho*g)))/2;
