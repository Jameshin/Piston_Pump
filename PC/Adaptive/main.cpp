...
void main()
{
	...
	void motion_RK4(double *phi, int mid, double hrem, double htry, double *hn, double *ym, double e, double kai, double gamma, double zeta, double de, double dkai,double dgamma, double dzeta, double **Hpc, double **Hn, double **Hs, double **Hw, double **He, double **Hpc_gro, 
	...
	double h, hout, hout_cy, hout_dy, hout_vb, hout_FM, hout_LK, hdid, hnext, hcal, hmcal, hn, hrem;
	...
	while(phi<phif){
		if(phi>=2.25*Pi){
			...
			/* Updating */
				...
				mid = 0;
				while(hcal<hout){
					hrem = (hout-hcal)*Pi/180;
					if(hrem < hn){
						hmcal = hrem;
						mid = 1;
					}			
					//printf("\n --------- %f %f \n",phit*180/Pi, hout*180/Pi);
					motion_RK4(&phit, mid, hrem, hmcal, &hn, ym, e, kai, gamma, zeta, de,  dkai, dgamma, dzeta, Hpc, Hn, Hs, Hw, He, Hpc_gro, 
...
void savedata(int *n, FILE *data0, FILE *data1, FILE *data2, FILE *data3, FILE *data4, FILE *data5, FILE *data6, double phi, double *hout_cy, double *hout_dy, double *hout_vb, double *hout_FM, double *hout_LK, double hout, double  *Pc, float **Zpc, float **Tpc, float **Ppc, double *ym, float *PCleakage, float **Acv, double **Hpc)
{
	...
	*hout_vb += hout;
	...
	if(*hout_dy >= phiout_dy){				
		...
		fprintf(data2, "%f %4.3e %4.3e %4.3e %4.3e %6.5e %4.3e %4.3e %4.3e %4.3e %4.3e %4.3e %4.3e %e %d %d \n", phi*180/Pi, sqrt(ym[1]*ym[1]+ym[2]*ym[2])*hp, atan2(ym[2],ym[1])*180/Pi, (sqrt(ym[3]*ym[3]+ym[4]*ym[4]))*hp/Rp*180/Pi, (-atan2(ym[3],ym[4]))*180/Pi, ym[1]*hp, ym[2]*hp, ym[3]*180/Pi*hp/Rp, ym[4]*180/Pi*hp/Rp, ym[5]*hp*w, ym[6]*hp*w, ym[7]*w*180/Pi*hp/Rp, ym[8]*w*180/Pi*hp/Rp, (/*ym[1]-(*gamma)*/Hmin)*hp, Imin, Jmin);
		*hout_dy = 0.0;
