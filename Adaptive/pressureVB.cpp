...
void filmthickness(double phi, double *ym, double **H, double **Hn, double **Hs, double **Hw, double **He, double **H_gro, float **Z, float **T, double e, double kai, double gamma, double zeta)
{
	...
	for(i=2; i<NNBz; i++){
		...
		Hw[i][1] = (H[i][1]+H[i][2])/2.0;
		He[i][1] = (H[i][1]+H[i][NNBt])/2.0;
	}
	// j=NNBt
	for(i=2; i<NNBz; i++){
		Hn[i][NNBt] = (H[i][NNBt]+H[i+1][NNBt])/2.0;
		Hs[i][NNBt] = (H[i][NNBt]+H[i-1][NNBt])/2.0;
		Hw[i][NNBt] = (H[i][NNBt]+H[i][1])/2.0;
		He[i][NNBt] = (H[i][NNBt]+H[i][NNBt-1])/2.0;
	}
	// i=1
	for(j=2; j<NNBt; j++){
	...
	Hw[1][1] = (H[1][1]+H[1][2])/2.0;
	He[1][1] = (H[1][1]+H[1][NNBt])/2.0;
	Hn[1][NNBt] = (H[1][NNBt]+H[2][NNBt])/2.0;
	Hw[1][NNBt] = (H[1][NNBt]+H[1][1])/2.0;
	He[1][NNBt] = (H[1][NNBt]+H[1][NNBt-1])/2.0;
	// i=NNBz
	for(j=2; j<NNBt; j++){
	...
	Hw[NNBz][1] = (H[NNBz][1]+H[NNBz][2])/2.0;
	He[NNBz][1] = (H[NNBz][1]+H[NNBz][NNBt])/2.0;
	Hs[NNBz][NNBt] = (H[NNBz][NNBt]+H[NNBz-1][NNBt])/2.0;
	Hw[NNBz][NNBt] = (H[NNBz][NNBt]+H[NNBz][1])/2.0;
	He[NNBz][NNBt] = (H[NNBz][NNBt]+H[NNBz][NNBt-1])/2.0;
}
/* FVM coefficient */
				...
				float **aN, float **aS, float **aW, float **aE, float **aC, float **B)
{
	...
	for(j=1; j<=NNBt; j++){
		for(i=2; i<NNBz; i++){
			// aN, aS, aW, aE
			aN[i][j] = pow(Hn[i][j],3)*dT[i][j]/dN[i][j];
			aS[i][j] = pow(Hs[i][j],3)*dT[i][j]/dS[i][j];
			aW[i][j] = pow(Hw[i][j],3)*dZ[i][j]/dW[i][j];
			aE[i][j] = pow(He[i][j],3)*dZ[i][j]/dE[i][j];
			...
			// Bwe, Bsq
			Bwe1[i][j] = (Hw[i][j]-He[i][j])*dZ[i][j];  // wedge
			Bwe2[i][j] = (Hn[i][j]-Hs[i][j])*dT[i][j]; 
			Bsq1[i][j] = -( (cos(Te[i][j]+kai)-cos(Tw[i][j]+kai))*de-e*(sin(Te[i][j]+kai)-sin(Tw[i][j]+kai))*dkai )*dZ[i][j]; // Normal squeeze
			Bsq2[i][j] = -(pow(Zn[i][j],2)-pow(Zs[i][j],2))/2*((sin(Te[i][j]+zeta)-sin(Tw[i][j]+zeta))*dgamma
...
void equation(float **P, float **Pold, float **aN, float **aS, float **aW, float **aE, float **aC, float **B)
{
	...
	// j = 1
	for(i=2; i<NNBz; i++){
		P[i][1] = (1-W)*Pold[i][1] + W*(aN[i][1]*P[i+1][1] + aS[i][1]*P[i-1][1] + aW[i][1]*P[i][2]   
					+ aE[i][1]*P[i][NNBt] - B[i][1]) / aC[i][1];
		if(P[i][1]<Pca/Lambda) P[i][1] = Pca/Lambda; 
	}
	//	j = NNBt	
	for(i=2; i<NNBz; i++){
		P[i][NNBt] = (1-W)*Pold[i][NNBt] + W*(aN[i][NNBt]*P[i+1][NNBt] + aS[i][NNBt]*P[i-1][NNBt] + aW[i][NNBt]*P[i][1]   
						+ aE[i][NNBt]*P[i][NNBt-1] - B[i][NNBt]) / aC[i][NNBt];
		if(P[i][NNBt]<Pca/Lambda) P[i][NNBt] = Pca/Lambda; 
	}
